...
const parameters = [
    ...
    const sheet = workbook.addWorksheet("Pass Level (XML)");
...
data.forEach(row => {
    ...
    let { pass, parameter } = row;

    const min = parseFloat(row.min);
    const max = parseFloat(row.max);

    // 🔹 Remove numeric suffix (Hot Pass 1 → Hot Pass)
    const basePass = pass.replace(/\s+\d+$/, "").trim();

    if (!passMap[basePass]) passMap[basePass] = {};
    if (!passMap[basePass][parameter]) {

        passMap[basePass][parameter] = {
            min: Infinity,
            max: -Infinity
        };
    ...
    if (!isNaN(min)) {
        passMap[basePass][parameter].min =
            Math.min(passMap[basePass][parameter].min, min);
    }

    if (!isNaN(max)) {
        passMap[basePass][parameter].max =
            Math.max(passMap[basePass][parameter].max, max);
    }
