...
// ... (constructor and parseAutomationFile remain the same)
    ...
    async run() {
        ...
        const mapRecord = (r, type) => {
            ...
            // 1. Get raw inputs for the formula
            const current = parseFloat(r[p + 'amps'] || 0);
            const volts = parseFloat(r[p + 'volts'] || 0);
            const travelSpeed = parseFloat(r.Travel_speed || 1); // Avoid division by zero

            // 2. Calculate Heat based on your formula
            // (0.06 * Current * Volts) / TravelSpeed
            ...
            return {
                ...
                'True Energy': this.applyRounding('Heat', r[p + 'heat']), // Original heat from TXT
                'Heat': this.applyRounding('Heat', calcHeat)            // Calculated heat
            };
        };
        ...
        console.log(`✅ Success: Heat calculated and True Energy stored.`);
    }
...
//     // --- FIX: Ensure 'groups' is defined here ---
//     const groups = {};
//     data.forEach(d => {
//         const key = groupFn(d);
...
//         // 3. Add the average values including True Energy and Calculated Heat
//         rowData.push(
